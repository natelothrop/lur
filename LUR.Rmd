...
```{r PCWS LUR Data Creation, echo=FALSE, message=FALSE, warning=FALSE}

# Create the annual average pollutant values
pcws_ap <- data %>%
  group_by(hhid,styear) %>%
  summarise(no2_adj=mean(no2_adj),
            pm25_adj=mean(pm25_adj),
            pm10_adj=mean(pm10_adj)) %>%
...
escape_lur_mod <- function(pollutant) {
  ...
  # Here is the model before removing predictors with pvalues > 0.1
  return(mod)
}
...
# Remove predictors one by one if their pvalue >0.1
pvaltest_preds <- tidy(mod)

while(any(pvaltest_preds$p.value > 0.1)){
  var_with_max_pvalue <- pvaltest_preds[(which(pvaltest_preds$p.value == max(pvaltest_preds$p.value))),1]  # get the var with max vif
    mod <- update(mod, ~. -caline_pm25)

  mod <- update(mod, ~. -as.symbol(var_with_max_pvalue))
  summary(mod)

  
  
  
  signif_all <- signif_all[!(signif_all) %in% var_with_max_pvalue]  # remove
  ...
  for (i in length(pred_coef$p.value)){
    
    
    # Pull out predictors one by one if the pvalue is >0.1, starting with the largest p-value until there no more predictors fit this condition
    
      # Arrange pvalues from largest to smallest
  pred_coef <- arrange(pred_coef, desc(p.value))
  
    
    if (pred_coef$p.value[i] > 0.1) {
      pred_coef[i,] <- NULL
      print("This predictor gets cut")
      
    } else {
      
      print("This predictor will stay around")
    }
  }
