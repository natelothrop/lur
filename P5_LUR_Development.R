...
data$no2ppb.ogawa <- (data$no2ppb * 0.68) + 13.53

data %>%
  filter(!is.na(no2ppb.ogawa)) %>%
  summarise(n.obs=length(no2ppb.ogawa),
            n.homes=n_distinct(hhid),
            geomean=exp(mean(log(no2ppb.ogawa))),
            geosd=exp(sd(log(no2ppb.ogawa))),
            min=min(no2ppb.ogawa),
            max=max(no2ppb.ogawa))

# By year
data %>%
  filter(!is.na(no2ppb) & styear==87) %>%
  summarise(n.obs=length(no2ppb),
            n.homes=n_distinct(hhid),
            geomean=exp(mean(log(no2ppb))),
            geosd=exp(sd(log(no2ppb))),
...
data %>%
  filter(!is.na(pm25ugm3) & styear==87) %>%
  summarise(n.obs=length(pm25ugm3),
            n.homes=n_distinct(hhid),
            geomean=exp(mean(log(pm25ugm3))),
            geosd=exp(sd(log(pm25ugm3))),
...
data %>%
  filter(!is.na(pm10ugm3) & styear==87) %>%
  summarise(n.obs=length(pm10ugm3),
            n.homes=n_distinct(hhid),
            geomean=exp(mean(log(pm10ugm3))),
            geosd=exp(sd(log(pm10ugm3))),
...
pm25freq_2obs <- data.frame(pm25freq_2obs)

pm25freq_87_2obs <- filter(pm25freq_2obs, styear==87)
...
pm10freq_2obs <- data.frame(pm10freq_2obs)
...
pm25freq_totobs <- data.frame(pm25freq_totobs)

pm25freq_totobs <- filter(pm25freq_totobs, freq==2)

pm25freq_totobs$styear <- NULL
pm25freq_totobs$freq <- NULL
pm25freq_totobs$stmon <- NULL

#pull out those with pm in two different seasons
pm10freq_totobs <- pm10freq_87_2obs %>%  
  group_by(hhid) %>%
  summarise(freq = n())

pm10freq_totobs <- data.frame(pm10freq_totobs)

pm10freq_totobs <- filter(pm10freq_totobs, freq==2)

pm10freq_totobs$styear <- NULL
pm10freq_totobs$freq <- NULL
pm10freq_totobs$stmon <- NULL
...
#create dataset of those with 2 obs in opposite seasons
rdata_2obs_pm25 <- merge(pm25freq_totobs,rdata, by=c("hhid"))
